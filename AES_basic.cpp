...
void genConstraints_of_Round(GRBModel& model, int r, unordered_map<string, vector<GRBVar>> e_map, unordered_map<string, vector<GRBVar>> er_map){
    ...
    for(int i=0; i<4; i++){       
        for (int j = 0; j < 4; j++){
            ...
            //for (int k = 0; k< 14; k++){
            //    cout<< i << " "<<j << " "<< k << " "<< deA_e_fAHAL[k].index() << endl;
            //}
    ...
    for (int i = 0; i < 4; i++){
        ...
        if (H__[i] == 1){ // identity: aaaa
            ...
            for (int j = 0; j < 4; j++){
                ...
                //model.addConstr(e_map["03"][perm[4*i+j]] == 0);
                //model.addConstr(e_map["12"][perm[4*i+j]] == 0);
            }
            
        }else if (H_c[i] == 1){ // copy: abab
            for (int j = 0; j < 4; j++){
                add_constraints_constant(model, &diag[j][0], "101101", 6);
                //model.addConstr(e_map["02"][perm[4*i+j]] == 0);
                //model.addConstr(e_map["13"][perm[4*i+j]] == 0);
            }
           
        } else { // shift: aacc
            for (int j = 0; j < 4; j++){
                add_constraints_constant(model, &diag[j][0], "011110", 6);
                //model.addConstr(e_map["01"][perm[4*i+j]] == 0);
                //model.addConstr(e_map["23"][perm[4*i+j]] == 0);
            }
        }
    }
...
/*
    ...
    for (int i = 0; i <= 15; i++){
        ...
        vector<GRBVar> var_list = {e_map["01"][i], e_map["02"][i], e_map["03"][i], e_map["12"][i], e_map["13"][i], e_map["23"][i], Bh[i], Bl[i]};
        add_constraints_from_template(model, var_list, e_ij_RandAct_pattern[0], 9, 9);
    }
    GRBLinExpr Bound = 0;
    for (int i = 0; i <= 15; i++){
        Bound += 2 * Bh[i];
...
*/
    //model.update();
    //----------end of bound variables -------------------

    // ----------gen ciphertext constraints---------------
    
    vector<int> d;
    if (r==4){
        d={0,1,2}; //collide on the d-th diagonal, e01=0, e23=0
    }
    else{
    ...
    if (model.get( GRB_IntAttr_Status ) == GRB_OPTIMAL){
		...
		{           
            ...
            //for (int ii = 0; ii < 3; ii++){
                ...
                //model.write("AES"+to_string(ii)+".sol");
                //cout<<ii<< " & ";
                //parse_sol(model, r, printMode);
                //input_patterns.push_back(input_pattern(model));
                //cout<<endl;
            }        			
		}
        else{ 
        ...
        //parse_inputpatterns(input_patterns);
